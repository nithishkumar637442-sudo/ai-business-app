...
app.post('/api/assistant', async (req, res) => {
  try {
    ...
    */
    ...
    /*
      Detect questions where missing customer information
      is important for personalization.
    */

    const customerRelatedQuestion =
      questionLower.includes('customer') ||
      questionLower.includes('customers') ||
      questionLower.includes('attract') ||
      questionLower.includes('daily customers') ||
      questionLower.includes('increase customers') ||
      questionLower.includes('more customers') ||
      questionLower.includes('target audience') ||
      questionLower.includes('marketing')

    /*
      Detect questions where budget information is important.
    */

    const budgetRelatedQuestion =
      questionLower.includes('budget') ||
      questionLower.includes('afford') ||
      questionLower.includes('spend') ||
      questionLower.includes('cost') ||
      questionLower.includes('costs') ||
      questionLower.includes('within') ||
      questionLower.includes('investment')

    /*
      Task 40:
      If important business data is missing, ask for it
      instead of allowing the AI to guess.

      We only ask when that information is actually relevant
      to the user's question.
    */

    if (
      isBusinessQuestion &&
      customerRelatedQuestion &&
      targetCustomers === 'Not provided'
    ) {
      const missingFields = []

      missingFields.push('target customers')

      if (
        budgetRelatedQuestion &&
        monthlyBudget === 'Not provided'
      ) {
        missingFields.push('monthly budget')
      }

      return res.json({
        reply:
          `To give you more personalized recommendations, ` +
    ...
    /*
      If the question specifically depends on budget
      and the budget is missing, ask for it instead of
      inventing an amount.
    */
    ...
    if (
      ...
      return res.json({
        reply:
          'To give you budget-aware recommendations, please provide your monthly budget.',
        budgetPlan: null,
      })
    }
...
- Never invent missing business profile information.
- If a business profile field is marked "Not provided",
  do not assume or guess its value.
